...
# Finite differences
fd=np.zeros([ny+3,5],DERIVS); matder=np.zeros([5,5],np.float64); rhsder=np.zeros([5],np.float64);
for iy in range(2,ny+1):
  for ir,ic in ( (ir,ic) for ir in range(0,5) for ic in range(0,5) ):
    matder[ir,ic] = (y[iy-2+ic] - y[iy])**(4.0-np.float64(ir))
  rhsder*=0; rhsder[3]=1;     fd[iy,:]['d1']=np.linalg.solve(matder,rhsder)
  rhsder*=0; rhsder[2]=2;     fd[iy,:]['d2']=np.linalg.solve(matder,rhsder)
fd[1,:]=der[1,:].copy();       fd[0,:]=der[0,:].copy()
fd[ny+1,:]=der[ny+1,:].copy(); fd[ny+2,:]=der[ny+2,:].copy()
