...
# gkei2 = np.memmap('help.bin',dtype = np.float64, mode='w+', shape=(ny//2, ny, nxc, nzc))
 ...
 for iry in range(0,nry):
  print(iry,'/',len(grid_ry_pl))
  with open(path+'rygkeibis%d.vtk'%(iry, ), "w") as vtkFile:
     ...
     print('DIMENSIONS',nxc//3,' ',nzc//3,' ',ny//2,file=vtkFile)
     ...
     print('Z_COORDINATES',ny//2,'float',file=vtkFile)
     for k in range(0,ny//2):
       print( grid_Yc_pl[k], file=vtkFile, sep=' ' )
     print('POINT_DATA ', (nxc//3)*(nzc//3)*(ny//2),file=vtkFile)
 #---------------------------------    SOURCE------------------------------------------------------------------------------
 ...
 for iry in range(0,nry):
      print('source-',iry,'/',len(grid_ry_pl))
      with open(path+'rygkeibis%d.vtk'%(iry, ), "a") as vtkFile:
        print('SCALARS source float 1',file=vtkFile)
        print('LOOKUP_TABLE default',file=vtkFile)
        for (i,j,k) in [(i,j,k) for k in  range(0,ny//2) for j in range(0,nzc,3) for i in range(0,nxc,3) ]:
         print(gkei2[k,iry,i,j], file=vtkFile, sep=' ' )
 print('fine source')
#------------------------------- phic ---------------------------------------------------------------------------------
# gkei2 = np.zeros([ny//2,ny,nxc,nzc],dtype='float64')
 print('interpolazione phic')
 for iz in range(0,nzc,3):
     print(iz,'/',nzc)
     start_time=time.time()
     for ix in range(0,nxc,3):
             print('phic-',iz,'-',ix,'/',nxc)
             gkei2[:,:,ix,iz]=np.nan_to_num(sp.interpolate.griddata(Y,gke[:,ix,iz]['Phic'], (grid_Yc, grid_ry), method='linear'))
              
     print(time.time()-start_time)
 print('appendo phiry ai file')
 for iry in range(0,nry):
      print('phic-',iry,'/',len(grid_ry_pl))
      with open(path+'rygkeibis%d.vtk'%(iry, ), "a") as vtkFile:
        print('SCALARS phic float 1',file=vtkFile)
        print('LOOKUP_TABLE default',file=vtkFile)
        for (i,j,k) in [(i,j,k) for k in  range(0,ny//2) for j in range(0,nzc,3) for i in range(0,nxc,3) ]:
         print(gkei2[k,iry,i,j], file=vtkFile, sep=' ' )
 print('fine phiry')
...
#-------------------------------------- PHIRX--------------------------------------------------------------------------------- 
 ...
 for iry in range(0,nry):
      print('phirx-',iry,'/',len(grid_ry_pl))
      with open(path+'rygkeibis%d.vtk'%(iry, ), "a") as vtkFile:
 ...
 for iry in range(0,nry):
      print('phirz-',iry,'/',len(grid_ry_pl))
      with open(path+'rygkeibis%d.vtk'%(iry, ), "a") as vtkFile:
